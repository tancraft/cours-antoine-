...
SELECT ROUND(AVG(`quantiteCommande`),2) AS `nombre moyen d'articles`,`descriptionArticle`,`nomClient`,`prenomClient` FROM `commandes` INNER JOIN `articles` INNER JOIN `clients` ON `commandes`.`idArticle` = `articles`.`idArticle` ON `commandes`.`idClient` = `articles`.`idArticle` GROUP BY `commandes`.`idArticle`
